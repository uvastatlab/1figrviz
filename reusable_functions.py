...
def make_freedom_collection_provider():
    """Checks subscribed title list from 'Elsevier_2019' file, subscribed journals list tab, (637 subscribed titles). Uses ISSN of each title and checks that against the
    UVA_1figr_original_dataset to see if ISSN is in any Elsevier title. If there is NOT a match, the title is assumed to be in the freedom collection.
...
#    original_1figr_dataset = pd.read_excel(filename, sheet_name='Journals per Provider', skiprows=8)
    ...
    freedom_collection_subset = subset_by_provider[subset_by_provider['ISSN/eISSN'].str.split(expand=True).isin(subscribed_journal_list_issns).any(1) == False]    #2193 titles
